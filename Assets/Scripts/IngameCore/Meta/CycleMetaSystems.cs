...
namespace ProjectW.IngameCore.Meta
{
    ...
    public static class PlaytestAnalyticsAggregator
    {
        public static PlaytestAnalyticsAggregate AggregateRecent(IReadOnlyList<PlaytestAnalyticsRecord> records, int windowSize)
        {
            ...
            {
                var entry = valid[i];
                if (IsAnnihilation(entry.TerminationReasonCode))
                {
                    annihilationCount += 1;
                }
                else
        ...
        private static bool IsAnnihilation(string terminationReasonCode)
        {
            ...
            var code = terminationReasonCode.Trim();
            return string.Equals(code, "TotalWipe", StringComparison.Ordinal)
                || string.Equals(code, "Annihilation", StringComparison.Ordinal)
                || string.Equals(code, "SessionEnd(Annihilation)", StringComparison.Ordinal);
        }
    }
}
