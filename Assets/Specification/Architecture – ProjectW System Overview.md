...
| **동기화 시각 (UTC)** | `2026-06-20T13:44:34Z` |
| **기준 커밋 (전체 SHA)** | `fc4dced21177fcc17c1c2cbc8c43156e3f8b8e19` |
| **기준 커밋 (단축)** | `fc4dced` |
| **브랜치** | `ai-integration` |
...
| Panel frame resource | `Assets/Resources/CaseReviewData/UI/Panel_Background_Frame.jpg`를 Sprite 9-slice(`spriteBorder` 140 px)로 import하고, `pixelsPerUnitMultiplier = 5`로 검정 테두리가 얇게 보이도록 surface panel과 button `Image.Type.Sliced` 배경에 적용 |
...
- `PanelFrameResourcePath = CaseReviewData/UI/Panel_Background_Frame`을 `Resources.Load<Sprite>`로 읽고, blocker/icon/handle을 제외한 surface panel과 button에는 `Image.Type.Sliced`와 `PanelFramePixelsPerUnitMultiplier = 5`로 얇은 hand-drawn frame 배경을 적용한다.
