...
## 6) 악세사리 장착 검증 체크리스트 (MVP 최소)

- [ ] **동시 장착**: 모자(`exclusiveGroup=hat`) + 안경 + 목걸이를 같은 캐릭터에 연속 장착해 모두 의도한 위치에 배치되는지 확인
- [ ] **그룹 충돌 정책**: 같은 `exclusiveGroup`(예: hat) 아이템 2개 연속 장착 시 정책이 동작하는지 확인
  - `ReplaceExisting`: 기존 아이템 해제 후 신규 장착 + 로그 출력
  - `RejectNew`: 기존 유지 + 신규 장착 거부 + 경고 로그 출력
- [ ] **좌우 반전**: 좌/우 반전 상태에서 `targetPart` + `anchorName` 우선 탐색이 유지되고, 미존재 시 파츠 루트 fallback이 적용되는지 확인
- [ ] **정렬 보장**: 악세사리 정렬값이 `targetPart`의 `Order in Layer + orderOffset`로 계산되어 항상 대상 파츠 위로 올라오는지 확인
- [ ] **이동 애니메이션 중 토글**: 이동 애니메이션 재생 중 장착/해제 반복 시 위치 튐, 소실, 고아 오브젝트 누수가 없는지 확인
