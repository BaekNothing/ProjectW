...
| Test ID | 검증 항목 | 측정 방식 | 목표값 (MVP) |
|---|---|---|---|
| U01 | 지연 개입 긴장감 체감 | 플레이 후 설문 5점 척도(4~5 비율) | 65% 이상 |
| U02 | 관계 붕괴 관찰 흥미 | 플레이 후 설문 5점 척도(4~5 비율) | 60% 이상 |
| U03 | 핵심 선택 인지율(언제/무엇/왜) | 세션 인터뷰에서 3요소 모두 언급 비율 | 70% 이상 |
| K01 | 1사이클 완료 시간 | 텔레메트리 중앙값 | 18분 ± 2분 |
| K02 | 사이클 완주율 | 전멸 제외 종료 비율 | 55% 이상 |
| K03 | 전멸 비율 | `SessionEnd(Annihilation)` 비율 | 45% 이하 |
| K04 | 실패 후 재도전율 | 실패 후 10분 내 재시작 비율 | 70% 이상 |
| K05 | 실패 후 즉시 이탈률 | 1판 종료 후 미재시작 비율 | 30% 이하 |

- UX 표본은 최소 2개 페르소나(전략 최적화형/서사 관찰형)를 균등하게 포함한다.
- KPI는 20세션 단위로 1차 판정하고, 50세션 누적 시 Go/No-Go 최종 판정을 갱신한다.
