...
- `work_definitions`, `cards`, `perks`, `characters`, `scenarios`
...
- `work_details`, `character_details`
  ...
  - `work_details` stores source truth frames with `actionCode` references, not authored visible logs or prose fact blobs; visible logs are generated from truth frames, current work state, and `truth_actions`.
...
- `truth_actions`
  - stores the action-code dictionary used by truth frames.
  - `work_details.truthFramesJson` references `actionCode`; descriptive visible text belongs in `truth_actions.visibleText`.
  - runtime visible logs are generated by combining the work state, truth frame identity/timing/actor, and the referenced action definition.
- `perks`
  - stores immutable perk authoring rows.
  - characters reference starting perks by `startingPerkIds`; current perk ownership remains gameplay progress and is saved locally.

Runtime rules:

- The client downloads `_manifest` first, then downloads enabled sheets over HTTPS.
- `localized_text`, `work_definitions`, `truth_actions`, `work_details`, `work_outcome_events`, `cards`, `perks`, `characters`, `character_details`, `scenarios`, and `scenario_details` are one required replacement snapshot.
